...
- run purpose:
  ...
  more explicit example structure can suppress the `destroyed_building3`
...
- replaced the broader mixed contrastive block with:
  - one dedicated `BUILDING TARGET-SELECTION EXAMPLES` block
  - one shorter `GENERAL CONTRASTIVE EXAMPLES` block
...
```bash
uv run bda-svc -i <run-root>/input_images -o <run-root>/qwen_candidate_v012
```
...
uv run python bda_eval/main.py \
  ...
  -p <run-root>/qwen_candidate_v012 \
...
- some bboxes tightened slightly on a few cases, but the core remaining target
  selection failures did not change

## Current Interpretation

This is not a clear upgrade over `v010`.

The example-heavy restructuring preserved the most important held gain, but it
did not change the unresolved failure family enough to justify keeping it live.

Working implication:

- example structure alone is not a strong enough next lever here
- `v012` should be recorded, then rejected
- the best current local Qwen state remains `v010`

## Companion Artifacts

- candidate version:
  - `../../versions/v012_detect_objects_example-structure-target-selection.yaml`
- parent-vs-candidate eval:

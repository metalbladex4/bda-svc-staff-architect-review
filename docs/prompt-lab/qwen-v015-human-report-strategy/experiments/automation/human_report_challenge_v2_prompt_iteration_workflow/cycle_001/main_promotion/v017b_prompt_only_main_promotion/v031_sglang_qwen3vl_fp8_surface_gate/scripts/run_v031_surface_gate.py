#!/usr/bin/env python3
"""Run the v031 official Qwen3-VL FP8 surface stability gate."""
...
PACKAGE_ROOT = (
    ...
    "v017b_prompt_only_main_promotion/v031_sglang_qwen3vl_fp8_surface_gate"
)
V020C_OVERLAY = (
    WORKTREE_ROOT
    / "docs/prompt-lab/qwen-v015-human-report-strategy/experiments/automation/"
...
def create_overlays() -> list[dict[str, Any]]:
    ...
    candidates = [
        ("v031a_case67_cold_exact_v020c_replay", v020c_prompt, []),
        ("v031b_case67_warmup_request", v020c_prompt, []),
        ("v031c_case67_warm_exact_v020c_replay_1", v020c_prompt, []),
        ("v031d_case67_warm_exact_v020c_replay_2", v020c_prompt, []),
        ("v031e_case67_warm_exact_v020c_replay_3", v020c_prompt, []),
        ("v031f_case67_blank_line_probe", blank_line_prompt, ["one extra blank line after the categories placeholder"]),
        ("v031g_case67_trailing_space_probe", trailing_space_prompt, ["one trailing space after the categories placeholder line"]),
        ("v031h_case67_noop_template_roundtrip", v020c_prompt, ["YAML roundtrip no-op; no intended semantic change"]),
        ("v031i_sentinel_exact_v020c_replay_1", v020c_prompt, []),
        ("v031j_sentinel_exact_v020c_replay_2", v020c_prompt, []),
        ("v031k_sentinel_blank_line_probe", blank_line_prompt, ["one extra blank line after the categories placeholder"]),
        ("v031l_sentinel_trailing_space_probe", trailing_space_prompt, ["one trailing space after the categories placeholder line"]),
        ("v031m_sentinel_noop_template_roundtrip", v020c_prompt, ["YAML roundtrip no-op; no intended semantic change"]),
    ]
    records = []
    for candidate_id, prompt, intended_changes in candidates:
        overlay = {
            ...
            "model_line": "Qwen/Qwen3-VL-8B-Instruct-FP8",
            "description": "v031 official Qwen3-VL FP8 surface repeatability probe.",
...
def create_scratch(candidate_id: str) -> Path:
    scratch = Path("/tmp") / f"bda_v031_{candidate_id}_{stamp()}"
    if scratch.exists():
...
def main() -> int:
    ...
    case67_manifest = PACKAGE_ROOT / "validation_manifests/v031_case67_only_no101.yaml"
    sentinel_manifest = PACKAGE_ROOT / "validation_manifests/v031_sentinel_micro_pack_no101.yaml"
    make_subset_manifest(["67"], case67_manifest, "v031_case67_only_no101")
    make_subset_manifest(
        ["12", "14", "16", "42", "66", "67", "77", "84", "88", "90", "97", "103", "155", "166", "172"],
        sentinel_manifest,
        "v031_sentinel_micro_pack_no101",
    )

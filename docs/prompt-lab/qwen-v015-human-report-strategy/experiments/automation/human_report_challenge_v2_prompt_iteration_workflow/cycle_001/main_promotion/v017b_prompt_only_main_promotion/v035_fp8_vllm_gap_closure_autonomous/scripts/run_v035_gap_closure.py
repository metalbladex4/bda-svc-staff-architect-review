#!/usr/bin/env python3
"""v035 FP8 vLLM gap-closure prompt-refinement operator."""
...
PACKAGE_ROOT = WORKTREE_ROOT / (
    ...
    "v017b_prompt_only_main_promotion/v035_fp8_vllm_gap_closure_autonomous"
)
...
SENTINEL_MANIFEST = PACKAGE_ROOT / "validation_manifests/v035_fp8_sentinel_with_110_no101.yaml"
...
def create_sentinel_manifest() -> None:
    ...
    write_yaml(
        SENTINEL_MANIFEST,
        {
            "pack_id": "v035_fp8_sentinel_with_110_no101",
...
def create_candidate_overlay() -> Path:
    ...
    addition = (
        "- isolated dense-scene marks or partial fragments when the target body center and exterior boundary are not both visible\n"
    )
    ...
    candidate["candidate_id"] = "v035a_fp8_dense_uncertain_fragment_guard"
    candidate["title"] = "v035a fp8 dense uncertain fragment guard"
    candidate["overlay_id"] = "qwen-1.2-v035a_fp8_dense_uncertain_fragment_guard"
    candidate["overlay_type"] = "fp8_vllm_gap_closure"
    candidate["description"] = "Gap-closure precision guard for residual dense-scene fragment FPs while preserving v034a and the v020c extra-box audit."
    candidate["generated_from"] = ["v034a_fp8_broad_context_scene_box_guard"]
    candidate["intended_changes"] = [
        "Add one compact BAD FINAL BOX guard for isolated dense-scene marks or partial fragments when target body center and exterior boundary are not both visible.",
    ...
    out = PACKAGE_ROOT / "overlays/v035a_fp8_dense_uncertain_fragment_guard.yaml"
...
def create_baseline_overlay_copy() -> Path:
    ...
    payload["candidate_id"] = "v035_baseline_exact_v034a_replay"
    payload["title"] = "v035 baseline exact v034a replay"
    payload["overlay_id"] = "qwen-1.2-v035_baseline_exact_v034a_replay"
    payload["overlay_type"] = "fp8_vllm_gap_closure"
    payload["description"] = "Exact FP8 v034a working-best replay for v035 sentinel verification."
    payload["intended_changes"] = []
    out = PACKAGE_ROOT / "overlays/v035_baseline_exact_v034a_replay.yaml"
    write_yaml(out, payload)
    return out


def create_scratch(candidate_id: str) -> Path:
    scratch = Path("/tmp") / f"bda_v035_{candidate_id}_{stamp()}"
    if scratch.exists():
...
def run_probe(candidate: dict[str, Any], manifest: Path, stage: str) -> dict[str, Any]:
    ...
    write_json(run_root / "v035_run_summary.json", record)
    return record
...
def print_candidate_block(record: dict[str, Any], stage: str, status: str, lesson: str, next_axis: str, office_ok: bool) -> None:
    ...
    print("=== V035 CANDIDATE COMPLETE ===", flush=True)
...
def write_scaffold(preflight: dict[str, Any]) -> None:
    for name in ["overlays", "diagnoses", "runs", "scripts", "research_notes.md", "visual_review_notes.md"]:
        ...
        if path.suffix:
            path.write_text(f"# v035 {path.stem.replace('_', ' ').title()}\n\n", encoding="utf-8")
        else:
    ...
    write_json(
        PACKAGE_ROOT / "source_manifest.json",
        {
            ...
            "package": "v035_fp8_vllm_gap_closure_autonomous",
            "source_artifacts": [
    ...
    (PACKAGE_ROOT / "README.md").write_text(
        "# v035 FP8 vLLM Gap Closure Autonomous\n\n"
        "Separate-model-line FP8 vLLM prompt-refinement tranche. The working best is `v034a_fp8_broad_context_scene_box_guard = 181 / 38 / 25 / 63`. "
...
def write_closeout(records: dict[str, Any], final_status: str, next_axis: str) -> None:
    ...
    diag = PACKAGE_ROOT / "diagnoses/v035a_fp8_dense_uncertain_fragment_guard_diagnosis.md"
    ...
    diag.write_text(
        ...
        "Known failure focus: residual case 66/67 dense FPs, case 84 recall, case 110 broad FP explosion, and controls 155/166/office-negative.\n\n"
...
def main() -> int:
    ...
    baseline = {"candidate_id": "v035_baseline_exact_v034a_replay", "overlay_path": str(baseline_overlay), "intended_changes": []}
    candidate = {
        "candidate_id": "v035a_fp8_dense_uncertain_fragment_guard",
    ...
    if not micro_ok:
        ...
        print_candidate_block(
            ...
            "micro_pack_only",
            status,
            f"v035a failed the FP8 micro gate: {micro_reason}.",
            "Pivot to visual review of residual dense fragments before another semantic prompt.",
            candidate_office_ok,
        )
        write_closeout(records, status, "Visual/eval artifact synthesis for residual dense fragments before another prompt candidate.")
        return 0
    ...
    print_candidate_block(
        ...
        "micro_pack_only",
        "learning_evidence",
        "v035a passed the FP8 micro gate; full all-current is now required.",
    ...
    lesson = "v035a completed full all-current on FP8 vLLM."
    next_axis = "Stop if v035a beats 63; otherwise pivot to narrower FP control from case-level full-run errors."

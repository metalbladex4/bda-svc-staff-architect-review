#!/usr/bin/env python3
"""v034 FP8 vLLM precision-recovery prompt-refinement operator."""
...
SENTINEL_MANIFEST = PACKAGE_ROOT / "validation_manifests/v034_fp8_sentinel_with_110_no101.yaml"
...
def create_sentinel_manifest() -> None:
    ...
    write_yaml(
        SENTINEL_MANIFEST,
        {
            "pack_id": "v034_fp8_sentinel_with_110_no101",
...
def create_candidate_overlay() -> Path:
    ...
    needle = "- extra neighboring box whose distinct body boundary is not visible\n"
    addition = (
    ...
    candidate["candidate_id"] = "v034a_fp8_broad_context_scene_box_guard"
    candidate["title"] = "v034a fp8 broad context scene box guard"
    candidate["overlay_id"] = "qwen-1.2-v034a_fp8_broad_context_scene_box_guard"
    candidate["overlay_type"] = "fp8_vllm_precision_recovery"
    candidate["description"] = "Precision-only FP8 guard against broad context/scene boxes while preserving the v020c extra-box audit."
    candidate["generated_from"] = ["v020c_fp8_vllm_baseline", "v033_v032d_case110_fp_explosion_lesson"]
    candidate["intended_changes"] = [
        "Add one BAD FINAL BOX guard for broad context or scene boxes whose strongest support is row/blast/road/debris/smoke/multiple uncertain fragments rather than one visible target body.",
    ...
    out = PACKAGE_ROOT / "overlays/v034a_fp8_broad_context_scene_box_guard.yaml"
...
def create_baseline_overlay_copy() -> Path:
    ...
    payload["candidate_id"] = "v034_baseline_exact_v020c_fp8_replay"
    payload["title"] = "v034 baseline exact v020c fp8 replay"
    payload["overlay_id"] = "qwen-1.2-v034_baseline_exact_v020c_fp8_replay"
    payload["overlay_type"] = "fp8_vllm_precision_recovery"
    payload["description"] = "Exact FP8 v020c baseline replay for v034 sentinel verification."
    payload["intended_changes"] = []
    out = PACKAGE_ROOT / "overlays/v034_baseline_exact_v020c_fp8_replay.yaml"
    write_yaml(out, payload)
    return out


def create_scratch(candidate_id: str) -> Path:
    scratch = Path("/tmp") / f"bda_v034_{candidate_id}_{stamp()}"
    if scratch.exists():
...
def run_probe(candidate: dict[str, Any], manifest: Path, stage: str) -> dict[str, Any]:
    ...
    write_json(run_root / "v034_run_summary.json", record)
    return record
...
def candidate_status(record: dict[str, Any], office_ok: bool) -> str:
    ...
    if errors < FP8_BASELINE_ERRORS:
        return "new_fp8_working_best"
    return "learning_evidence" if errors <= FP8_BASELINE_ERRORS else "rejected"
...
def print_candidate_block(record: dict[str, Any], stage: str, status: str, lesson: str, next_axis: str, office_ok: bool) -> None:
    ...
    print("=== V034 CANDIDATE COMPLETE ===", flush=True)
...
def write_scaffold(preflight: dict[str, Any]) -> None:
    for name in ["overlays", "diagnoses", "runs", "scripts", "research_notes.md", "visual_review_notes.md"]:
        ...
        if path.suffix:
            path.write_text(f"# v034 {path.stem.replace('_', ' ').title()}\n\n", encoding="utf-8")
        else:
    ...
    write_json(
        PACKAGE_ROOT / "source_manifest.json",
        {
            ...
            "package": "v034_fp8_vllm_precision_recovery_autonomous",
            "source_artifacts": [
            ...
            "hard_boundaries": {"no_promotion": True, "no_product_truth_mutation": True, "no_v024o": True, "no_v032d_as_base": True},
        },
    )
    write_json(
        PACKAGE_ROOT / "model_line_manifest.json",
        {
            ...
            "backend": BACKEND,
            "working_best_start": "v020c_fp8_vllm_baseline",
            "working_best_metrics": {"matches": 180, "false_negatives": 39, "false_positives": 32, "combined_errors": 71},
            "not_product_replacement": True,
        },
    )
    (PACKAGE_ROOT / "README.md").write_text(
        "# v034 FP8 vLLM Precision Recovery Autonomous\n\n"
        "Separate-model-line FP8 vLLM prompt-refinement tranche. The working baseline is `v020c_fp8_vllm_baseline = 180 / 39 / 32 / 71`. "
        "`v032d` is rejected learning evidence only and is not used as the base prompt.\n",
        encoding="utf-8",
    )
...
def write_closeout(records: dict[str, Any], final_status: str, next_axis: str) -> None:
    ...
    (PACKAGE_ROOT / "comparison_matrix.md").write_text(
        ...
        "|---|---|---:|---:|---:|---:|---|---|---|---|---|---|\n"
        + "\n".join(
            f"| `{row['candidate_id']}` | {row['stage']} | {row['metrics']['matches']} | {row['metrics']['false_negatives']} | {row['metrics']['false_positives']} | {row['metrics']['combined_errors']} | `{row.get('case_67', 'n/a')}` | `{row.get('case_110', 'n/a')}` | `{row.get('case_155', 'n/a')}` | `{row.get('case_166', 'n/a')}` | {row.get('office_negative', 'n/a')} | {row['status']} |"
            for row in rows
        )
        + "\n",
        encoding="utf-8",
    ...
    best_candidate = "v020c_fp8_vllm_baseline"
    beat_baseline = False
    beat_old = False
    target_met = False
    if final_record and not final_record.get("runtime_invalid"):
        errors = final_record["metrics"]["combined_errors"]
        if errors < FP8_BASELINE_ERRORS and records.get("candidate_full_status") in {"new_fp8_working_best", "beat_old_v020c_reference", "target_met"}:
    ...
    (PACKAGE_ROOT / "lessons_learned.md").write_text(
        ...
        "- Precision candidates must preserve v020c audit discipline and avoid recall-loosening language.\n",
        encoding="utf-8",
    )
    ...
    (PACKAGE_ROOT / "recovery_log.md").write_text(
        "# v034 Recovery Log\n\n"
        "- Re-grounded in v032/v033 evidence.\n"
        "- Used v033 timeout/retry policy for FP8 vLLM.\n"
        "- Created sentinel manifest including case 110.\n"
        "- Authored one compact precision candidate from the FP8 baseline, not from v032d.\n",
        encoding="utf-8",
    )
    write_json(PACKAGE_ROOT / "recovery_log.json", {"generated_at": utc_now(), "records": sorted(records.keys())})
    diag = PACKAGE_ROOT / "diagnoses/v034a_fp8_broad_context_scene_box_guard_diagnosis.md"
    ...
    diag.write_text(
        ...
        "Known failure focus: case 110 broad FP explosion, dense cases 66/67/84/97, and controls 155/166/office-negative.\n\n"
...
def main() -> int:
    ...
    baseline = {"candidate_id": "v034_baseline_exact_v020c_fp8_replay", "overlay_path": str(baseline_overlay), "intended_changes": []}
    candidate = {
    ...
    if not micro_ok:
        ...
        print_candidate_block(
            ...
            "micro_pack_only",
            status,
            f"v034a failed the FP8 micro gate: {micro_reason}.",
            "Pivot to a narrower case-110 visual failure-class review before another semantic prompt.",
            candidate_office_ok,
        )
        write_closeout(records, status, "Case-110 visual/eval artifact synthesis before another prompt candidate.")
        return 0
    ...
    print_candidate_block(
        ...
        "micro_pack_only",
        "learning_evidence",
        "v034a passed the FP8 micro gate; full all-current is now required.",
        "Run full all-current/no101 before deciding whether it beats the FP8 baseline.",
        candidate_office_ok,
    )
    ...
    lesson = "v034a completed full all-current on FP8 vLLM."
    next_axis = "Continue only if v034a beats 71; otherwise pivot to narrower FP control from the case-level full-run errors."

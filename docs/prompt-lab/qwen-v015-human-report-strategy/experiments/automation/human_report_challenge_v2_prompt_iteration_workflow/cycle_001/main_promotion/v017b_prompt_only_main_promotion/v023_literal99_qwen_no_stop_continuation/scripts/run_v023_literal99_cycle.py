#!/usr/bin/env python3
"""Run the v023 literal-99 Qwen prompt refinement cycle.
...
def _create_scratch(row_id: str, upstream_commit: str) -> Path:
    ...
    scratch = SCRATCH_PARENT / f"_scratch_v023_{row_id}_{stamp}"
...
def _load_matrix() -> dict[str, Any]:
    ...
    return {
        "matrix_id": "v023_literal99_qwen_no_stop_continuation",
        "generated_utc": _utc(),
        "results": [],
        "rankings": [],
    }
...
def _write_matrix_md(matrix: dict[str, Any]) -> None:
    lines = [
        "# v023 Literal-99 Qwen Comparison Matrix",
        "",
...
def init_package() -> int:
    ...
    readme = f"""# v023 Literal-99 Qwen Recursive Prompt Refinement Cycle

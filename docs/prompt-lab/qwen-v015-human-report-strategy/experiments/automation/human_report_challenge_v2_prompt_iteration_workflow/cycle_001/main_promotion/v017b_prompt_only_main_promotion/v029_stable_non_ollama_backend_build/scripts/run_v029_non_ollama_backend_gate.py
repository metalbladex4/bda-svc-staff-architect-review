#!/usr/bin/env python3
"""Run the v029 non-Ollama backend stability and baseline gate."""
...
def create_overlays() -> list[dict[str, Any]]:
    ...
    candidates = [
        ("v029a_case67_exact_v020c_replay_1", v020c_prompt, []),
        ("v029b_case67_exact_v020c_replay_2", v020c_prompt, []),
        ("v029c_case67_exact_v020c_replay_3", v020c_prompt, []),
        ("v029d_case67_blank_line_probe_1", blank_line_prompt, ["one extra blank line after the categories placeholder"]),
        ("v029e_case67_blank_line_probe_2", blank_line_prompt, ["one extra blank line after the categories placeholder"]),
        ("v029f_case67_trailing_space_probe", trailing_space_prompt, ["one trailing space after the categories placeholder line"]),
        ("v029g_case67_noop_template_roundtrip", v020c_prompt, ["YAML roundtrip no-op; no intended semantic change"]),
        ("v029h_sentinel_exact_v020c_replay_1", v020c_prompt, []),
        ("v029i_sentinel_exact_v020c_replay_2", v020c_prompt, []),
        ("v029j_sentinel_blank_line_shape_probe", blank_line_prompt, ["one extra blank line after the categories placeholder"]),
        ("v029k_sentinel_trailing_space_shape_probe", trailing_space_prompt, ["one trailing space after the categories placeholder line"]),
        ("v029l_sentinel_noop_template_roundtrip", v020c_prompt, ["YAML roundtrip no-op; no intended semantic change"]),
    ]
    records = []
    for candidate_id, prompt, intended_changes in candidates:
        overlay = {
            ...
            "description": "v029 stable-backend recovery and repeatability probe.",
...
def create_scratch(candidate_id: str) -> Path:
    scratch = Path("/tmp") / f"bda_v029_{candidate_id}_{stamp()}"
    if scratch.exists():
...
def run_probe(candidate: dict[str, Any], manifest: Path, backend: dict[str, str], stage: str, preflight: dict[str, Any]) -> dict[str, Any]:
    ...
    if rendered_text:
        if not base_rendered_path.exists() or candidate_id == "v029a_case67_exact_v020c_replay_1":
...
def main() -> int:
    ...
    parser.add_argument("--run", action="store_true", help="Run v029 non-Ollama backend gate.")
    ...
    case67_manifest = PACKAGE_ROOT / "validation_manifests/v029_case67_only_no101.yaml"
    sentinel_manifest = PACKAGE_ROOT / "validation_manifests/v029_sentinel_micro_pack_no101.yaml"
    make_subset_manifest(["67"], case67_manifest, "v029_case67_only_no101")
    make_subset_manifest(
        ["12", "14", "16", "42", "66", "67", "77", "84", "88", "90", "97", "103", "155", "166", "172"],
        sentinel_manifest,
        "v029_sentinel_micro_pack_no101",
    )

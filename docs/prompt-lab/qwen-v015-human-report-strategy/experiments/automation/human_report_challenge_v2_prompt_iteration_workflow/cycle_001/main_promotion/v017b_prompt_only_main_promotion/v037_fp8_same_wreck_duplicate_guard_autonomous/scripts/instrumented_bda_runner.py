...
REQUEST_TIMEOUT_SECONDS = float(os.getenv("BDA_VLM_REQUEST_TIMEOUT_SECONDS", "60"))
...
def _make_instrumented_generate() -> Any:
    def instrumented_generate(
        ...
        sanitized_request = {
            ...
            "max_tokens": None,
            "stop": None,
            "timeout": f"OpenAI client timeout={REQUEST_TIMEOUT_SECONDS:g} from v033 experiment-only instrumentation",
            "images": image_trace,
        }
        ...
        try:
            client = self.client.with_options(timeout=REQUEST_TIMEOUT_SECONDS)

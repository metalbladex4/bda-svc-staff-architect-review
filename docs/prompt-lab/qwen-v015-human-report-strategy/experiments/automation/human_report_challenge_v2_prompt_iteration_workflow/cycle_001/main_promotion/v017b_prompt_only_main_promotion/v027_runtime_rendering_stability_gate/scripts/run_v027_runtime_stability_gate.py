#!/usr/bin/env python3
"""Run the v027 runtime-rendering/request-shape stability gate."""
...
PACKAGE_ROOT = (
    ...
    "v017b_prompt_only_main_promotion/v027_runtime_rendering_stability_gate"
)
V020C_OVERLAY = (
    WORKTREE_ROOT
    / "docs/prompt-lab/qwen-v015-human-report-strategy/experiments/automation/"
...
def select_backend() -> tuple[dict[str, str], dict[str, Any]]:
    ...
    if preferred_status["ok"]:
        ...
        reason = "preferred endpoint available"
    elif fallback_status["ok"]:
        selected = FALLBACK_BACKEND
        reason = "preferred endpoint unavailable; authorized fallback available"
    else:
        selected = {}
...
def ensure_scaffold() -> None:
    for subdir in ["overlays", "runs", "scripts", "diagnoses", "validation_manifests"]:
    ...
    for name, payload in {
        ...
        "stability_matrix.json": {"generated_at": utc_now(), "records": []},
...
def update_manifest_files(record: dict[str, Any]) -> None:
    ...
    matrix_path = PACKAGE_ROOT / "stability_matrix.json"
    ...
    append_text(
        PACKAGE_ROOT / "stability_matrix.md",
...
def create_overlays() -> list[dict[str, Any]]:
    ...
    candidates = [
        ("v027a_case67_exact_v020c_replay_1", v020c_prompt, []),
        ("v027b_case67_exact_v020c_replay_2", v020c_prompt, []),
        ("v027c_case67_exact_v020c_replay_3", v020c_prompt, []),
        ("v027d_case67_blank_line_probe_1", blank_line_prompt, ["one extra blank line after the categories placeholder"]),
        ("v027e_case67_blank_line_probe_2", blank_line_prompt, ["one extra blank line after the categories placeholder"]),
        ("v027f_case67_trailing_space_probe", trailing_space_prompt, ["one trailing space after the categories placeholder line"]),
        ("v027g_case67_noop_template_roundtrip", v020c_prompt, ["YAML roundtrip no-op; no intended semantic change"]),
        ("v027h_sentinel_exact_v020c_replay_1", v020c_prompt, []),
        ("v027i_sentinel_exact_v020c_replay_2", v020c_prompt, []),
        ("v027j_sentinel_blank_line_shape_probe", blank_line_prompt, ["one extra blank line after the categories placeholder"]),
        ("v027k_sentinel_trailing_space_shape_probe", trailing_space_prompt, ["one trailing space after the categories placeholder line"]),
        ("v027l_sentinel_noop_template_roundtrip", v020c_prompt, ["YAML roundtrip no-op; no intended semantic change"]),
    ]
    records = []
    for candidate_id, prompt, intended_changes in candidates:
        overlay = {
            ...
            "overlay_type": "runtime_stability_probe",
            "model_line": "qwen3-vl:8b-instruct",
            "description": "v027 runtime-rendering/request-shape stability probe.",
...
def create_scratch(candidate_id: str) -> Path:
    scratch = Path("/tmp") / f"bda_v027_{candidate_id}_{stamp()}"
    if scratch.exists():
...
def print_candidate_block(record: dict[str, Any]) -> None:
    print("=== CANDIDATE COMPLETE ===", flush=True)
    print(f"candidate: {record['candidate_id']}", flush=True)
    print(f"backend: {record['backend_label']}", flush=True)
    print(f"stage: {record['stage']}", flush=True)
    print(f"matches: {record['matches']}", flush=True)
    print(f"false_negatives: {record['false_negatives']}", flush=True)
    print(f"false_positives: {record['false_positives']}", flush=True)
    print(f"combined_errors: {record['combined_errors']}", flush=True)
    print(f"vs_v020c_errors_delta: {record['vs_v020c_errors_delta']}", flush=True)
    print(f"case_67: {record['case_67']}", flush=True)
    print(f"case_155: {record['case_155']}", flush=True)
    print(f"case_166: {record['case_166']}", flush=True)
    print(f"office_negative: {record['office_negative']}", flush=True)
    ...
    print(f"response_trace_captured: {str(record['response_trace_captured']).lower()}", flush=True)
    print(f"status: {record['status']}", flush=True)
    print(f"main_lesson: {record['main_lesson']}", flush=True)
    print(f"next_axis: {record['next_axis']}", flush=True)
    print("==========================", flush=True)


def run_probe(candidate: dict[str, Any], manifest: Path, backend: dict[str, str], stage: str) -> dict[str, Any]:
    ...
    if rendered_text:
        if not base_rendered_path.exists() or candidate_id == "v027a_case67_exact_v020c_replay_1":
...
def main() -> int:
    ...
    parser.add_argument("--run", action="store_true", help="Run v027 stability gate.")
    ...
    case67_manifest = PACKAGE_ROOT / "validation_manifests/v027_case67_only_no101.yaml"
    sentinel_manifest = PACKAGE_ROOT / "validation_manifests/v027_sentinel_micro_pack_no101.yaml"
    make_subset_manifest(["67"], case67_manifest, "v027_case67_only_no101")
    make_subset_manifest(
        ["12", "14", "16", "42", "66", "67", "77", "84", "88", "90", "97", "103", "155", "166", "172"],
        sentinel_manifest,
        "v027_sentinel_micro_pack_no101",
    )
    ...
    try:
        for candidate in stage1_candidates:
            stage1_records.append(run_probe(candidate, case67_manifest, backend, "case67_stability"))
    finally:
        ...
        if stage1_pass:
            ...
            for candidate in stage2_candidates:
                stage2_records.append(run_probe(candidate, sentinel_manifest, backend, "sentinel_stability"))

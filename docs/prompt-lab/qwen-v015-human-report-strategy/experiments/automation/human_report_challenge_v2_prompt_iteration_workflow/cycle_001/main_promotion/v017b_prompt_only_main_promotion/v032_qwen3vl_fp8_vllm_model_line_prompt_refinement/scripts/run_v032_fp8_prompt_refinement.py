#!/usr/bin/env python3
"""Run the v032 Qwen3-VL FP8 vLLM model-line prompt refinement gate."""
...
PACKAGE_ROOT = (
    ...
    "v017b_prompt_only_main_promotion/v032_qwen3vl_fp8_vllm_model_line_prompt_refinement"
)
V020C_OVERLAY = (
    WORKTREE_ROOT
    / "docs/prompt-lab/qwen-v015-human-report-strategy/experiments/automation/"
...
PREFERRED_BACKEND = {
    "label": "vllm_qwen3vl_8b_fp8_local_8000",
...
def select_backend() -> tuple[dict[str, str], dict[str, Any]]:
    ...
    if preferred_status["ok"] and model_list_has(preferred_status, PREFERRED_BACKEND["model"]):
        ...
        reason = (
            "vLLM official Qwen3-VL FP8 endpoint available on localhost:8000 with expected "
    ...
    sglang_version = run(
        [
            "bash",
            "-lc",
            "/tmp/bda_v032_sglang_env/bin/python - <<'PY'\nimport importlib.metadata as md\nfor p in ['sglang','torch','transformers','openai','fastapi','uvicorn','compressed-tensors','sgl-kernel','flashinfer-python','nvidia-cudnn-cu12']:\n    try: print(p, md.version(p))\n    except Exception: print(p, 'unavailable')\nPY",
        ],
        CAPSTONE_ROOT,
    )
    package_probe = run(
...
def create_overlays() -> list[dict[str, Any]]:
    ...
    precision_guard_prompt = v020c_prompt.replace(
        "- extra neighboring box whose distinct body boundary is not visible\n",
        "- extra neighboring box whose distinct body boundary is not visible\n"
        "- strongest evidence is only smoke, debris, road/terrain texture, roof/facade patch, or a cropped fragment rather than a visible target body center\n",
    )
    compact_json_prompt = v020c_prompt.replace(
        ...
        "FP8 model-line calibration: keep the v020c visual standard, but be stricter about final boxes that lack a visible target body center.\n\n",
    )
    candidates = [
        ("v032_baseline_exact_v020c_fp8_replay", v020c_prompt, []),
        ("v032a_fp8_context_fragment_precision_guard", precision_guard_prompt, ["Add one bad-box guard against context-only, facade/roof-patch, terrain, and cropped-fragment boxes in BAD FINAL BOX."]),
        ("v032b_fp8_real_context_fragment_precision_guard", precision_guard_prompt, ["Retry v032a after detecting that v032a rendered as a no-op; actual added bad-box guard against context-only, facade/roof-patch, terrain, and cropped-fragment boxes."]),
        ("v032c_fp8_compact_calibration", compact_json_prompt, ["Replace the v020c historical calibration sentence with a shorter FP8 model-line calibration sentence while preserving the rest of the prompt."]),
    ]
    records = []
    for candidate_id, prompt, intended_changes in candidates:
        overlay = {
            ...
            "overlay_type": "fp8_vllm_model_line_prompt_refinement",
            "model_line": "qwen3-vl-8b-instruct-fp8-vllm",
            "description": "v032 separate FP8 vLLM model-line prompt candidate.",
...
def create_scratch(candidate_id: str) -> Path:
    scratch = Path("/tmp") / f"bda_v032_{candidate_id}_{stamp()}"
    if scratch.exists():
...
def run_probe(candidate: dict[str, Any], manifest: Path, backend: dict[str, str], stage: str, preflight: dict[str, Any]) -> dict[str, Any]:
    ...
    if rendered_text:
        if not base_rendered_path.exists() or candidate_id == "v031a_case67_cold_exact_v020c_replay":
...
def write_inventory_docs(preflight: dict[str, Any]) -> None:
    backend_inventory = {
        ...
        "vllm_launch_doc": "Project README documents: vllm serve Qwen/Qwen3-VL-8B-Instruct-FP8",
        "vllm_feasibility": "vLLM/SGLang/Transformers were not installed; Qwen/Qwen3-VL-8B-Instruct-FP8 is public/apache-2.0 but not cached locally and full BF16 8.8B weights are not treated as safe to pull/load blindly on 16GB VRAM during this tranche.",
    }
...
def main() -> int:
    ...
    case67_manifest = PACKAGE_ROOT / "validation_manifests/v032_case67_only_no101.yaml"
    sentinel_manifest = PACKAGE_ROOT / "validation_manifests/v032_sentinel_micro_pack_no101.yaml"
    make_subset_manifest(["67"], case67_manifest, "v032_case67_only_no101")
    make_subset_manifest(
        ["12", "14", "16", "42", "66", "67", "77", "84", "88", "90", "97", "103", "155", "166", "172"],
        sentinel_manifest,
        "v032_sentinel_micro_pack_no101",
    )
    candidates = create_overlays()
    stage1_candidates = candidates[:8]
    stage2_candidates = candidates[8:]

#!/usr/bin/env python3
"""Run the v028 stable-backend recovery and repeatability gate."""
...
PACKAGE_ROOT = (
    ...
    "v017b_prompt_only_main_promotion/v028_stable_backend_recovery_and_prompt_resume"
)
V020C_OVERLAY = (
    WORKTREE_ROOT
    / "docs/prompt-lab/qwen-v015-human-report-strategy/experiments/automation/"
...
PREFERRED_BACKEND = {
    "label": "upstream_openai_compat_preferred_8000",
...
def select_backend() -> tuple[dict[str, str], dict[str, Any]]:
    ...
    deterministic_create = None
    deterministic_8000_launch = None
    deterministic_11434_status = None
    if not preferred_status["ok"] and fallback_status["ok"]:
        deterministic_create = create_deterministic_ollama_model()
        deterministic_11434_status = fetch_url(FALLBACK_BACKEND["base_url"] + "/models")
        if deterministic_create.get("ok"):
            deterministic_8000_launch = launch_ollama_on_8000()
            preferred_status = fetch_url(PREFERRED_BACKEND["base_url"] + "/models")

    if preferred_status["ok"] and model_list_has(preferred_status, PREFERRED_BACKEND["model"]):
        selected = PREFERRED_BACKEND
        reason = "original preferred endpoint available with expected Qwen/Qwen3-VL model"
    elif preferred_status["ok"] and model_list_has(preferred_status, DETERMINISTIC_OLLAMA_MODEL):
        selected = DETERMINISTIC_OLLAMA_8000
        reason = "original preferred endpoint unavailable; launched deterministic Ollama-backed local endpoint on port 8000"
    elif deterministic_11434_status and model_list_has(deterministic_11434_status, DETERMINISTIC_OLLAMA_MODEL):
        selected = DETERMINISTIC_OLLAMA_11434
        reason = "preferred endpoint unavailable; deterministic Ollama model available on fallback endpoint"
    elif fallback_status["ok"]:
        selected = FALLBACK_BACKEND
        reason = "preferred endpoint unavailable; only original authorized fallback available"
    else:
        selected = {}
        reason = "no authorized endpoint available"
    ollama_version = run(["ollama", "--version"], CAPSTONE_ROOT)
    vllm_version = run(["bash", "-lc", "command -v vllm >/dev/null && vllm --version || true"], CAPSTONE_ROOT)
    sglang_version = run(["bash", "-lc", "python3 - <<'PY'\nimport importlib.metadata as md\nfor p in ['vllm','sglang','transformers','accelerate','torch','openai','fastapi','uvicorn','litellm']:\n    try: print(p, md.version(p))\n    except Exception as e: print(p, 'unavailable')\nPY"], CAPSTONE_ROOT)
    preflight = {
        "generated_at": utc_now(),
        "preferred_endpoint_status": preferred_status,
        "fallback_endpoint_status": fallback_status,
        "deterministic_ollama_create": deterministic_create,
        "deterministic_ollama_8000_launch": deterministic_8000_launch,
        "deterministic_ollama_11434_status": deterministic_11434_status,
        "selected_backend": selected,
        "backend_selected_reason": reason,
        "ollama_version": ollama_version if fallback_status["ok"] else None,
        "vllm_version": vllm_version,
        "python_package_probe": sglang_version,
    }
...
def create_overlays() -> list[dict[str, Any]]:
    ...
    candidates = [
        ("v028a_case67_exact_v020c_replay_1", v020c_prompt, []),
        ("v028b_case67_exact_v020c_replay_2", v020c_prompt, []),
        ("v028c_case67_exact_v020c_replay_3", v020c_prompt, []),
        ("v028d_case67_blank_line_probe_1", blank_line_prompt, ["one extra blank line after the categories placeholder"]),
        ("v028e_case67_blank_line_probe_2", blank_line_prompt, ["one extra blank line after the categories placeholder"]),
        ("v028f_case67_trailing_space_probe", trailing_space_prompt, ["one trailing space after the categories placeholder line"]),
        ("v028g_case67_noop_template_roundtrip", v020c_prompt, ["YAML roundtrip no-op; no intended semantic change"]),
        ("v028h_sentinel_exact_v020c_replay_1", v020c_prompt, []),
        ("v028i_sentinel_exact_v020c_replay_2", v020c_prompt, []),
        ("v028j_sentinel_blank_line_shape_probe", blank_line_prompt, ["one extra blank line after the categories placeholder"]),
        ("v028k_sentinel_trailing_space_shape_probe", trailing_space_prompt, ["one trailing space after the categories placeholder line"]),
        ("v028l_sentinel_noop_template_roundtrip", v020c_prompt, ["YAML roundtrip no-op; no intended semantic change"]),
    ]
    records = []
    for candidate_id, prompt, intended_changes in candidates:
        overlay = {
            ...
            "overlay_type": "stable_backend_recovery_probe",
            "model_line": "qwen3-vl:8b-instruct",
            "description": "v028 stable-backend recovery and repeatability probe.",
...
def create_scratch(candidate_id: str) -> Path:
    scratch = Path("/tmp") / f"bda_v028_{candidate_id}_{stamp()}"
    if scratch.exists():
...
def run_probe(candidate: dict[str, Any], manifest: Path, backend: dict[str, str], stage: str, preflight: dict[str, Any]) -> dict[str, Any]:
    ...
    if rendered_text:
        if not base_rendered_path.exists() or candidate_id == "v028a_case67_exact_v020c_replay_1":
...
def main() -> int:
    ...
    parser.add_argument("--run", action="store_true", help="Run v028 stable-backend gate.")
    ...
    case67_manifest = PACKAGE_ROOT / "validation_manifests/v028_case67_only_no101.yaml"
    sentinel_manifest = PACKAGE_ROOT / "validation_manifests/v028_sentinel_micro_pack_no101.yaml"
    make_subset_manifest(["67"], case67_manifest, "v028_case67_only_no101")
    make_subset_manifest(
        ["12", "14", "16", "42", "66", "67", "77", "84", "88", "90", "97", "103", "155", "166", "172"],
        sentinel_manifest,
        "v028_sentinel_micro_pack_no101",
    )
